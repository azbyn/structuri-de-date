...
public:
    ...
    static void test() {
        using FP = FixedPoint<2>;

        static_assert(FP(1.2).val == 120);
        static_assert(FP(1).val == 100);
        static_assert((FP(1.2) * FP(2.0)).val == 240);
        static_assert((FP(1.2) / FP(2.0)).val == 60);
        static_assert((FP(1.2) + FP(2.0)).val == 320);
        static_assert((FP(1.2) - FP(2.0)).val == -80);
        static_assert((FP(2.0) - FP(1.2)).val == 80);
        try {

            auto testStr = [] (const char* str, const char* val) {
                assert(FP::fromString(str).str == val, str);
            };
            auto testVal = [] (const char* str, long val) {
                FP f;
                if (FP::fromString(str, f) == nullptr) {
                    std::cerr << "'" << str << "' returned nullptr; might be bad\n";
                }
                if (f.val != val) {
                    std::cerr << "(" << f.val << "!="<<val << ")\n";
                    assert(false, str);
